...
function downloadReport(data, total, grade) {
  ...
  const feedbackItems = data.feedback
    ...
    .join("");
  const complianceRows = Object.entries(COMPLIANCE_LABELS)
    .map(([key, label]) => {
      const entry = data.compliance?.[key];
      const mark = entry?.present ? "✓" : "✗";
      const color = entry?.present ? "#1a7f4f" : "#b0413e";
      const note = entry?.note ? escapeHtml(entry.note) : "";
      return `<tr><td>${escapeHtml(label)}</td><td style="color:${color};font-weight:700">${mark}</td><td>${note}</td></tr>`;
    })
    .join("");
  const report = `<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><title>AI Presentation Reviewer Report</title>
<style>body{max-width:760px;margin:40px auto;padding:0 24px;color:#17212b;font:16px/1.6 Arial,sans-serif}h1{margin-bottom:4px;color:#12608b}h2{margin-top:32px;color:#12608b}p{color:#536575}.summary{padding:20px;border:1px solid #b8d8e8;border-radius:10px;background:#eef9ff}.score{font-size:32px;font-weight:700;color:#12608b}table{width:100%;border-collapse:collapse}td{padding:10px;border-bottom:1px solid #d9e5eb}td:last-child{text-align:left}table tr td:nth-child(2){text-align:right;font-weight:700}li{margin:10px 0}</style>
...
function renderCompliance(compliance) {
  if (!complianceList) return; // HTML container not added yet — skip safely
  ...
  for (const [key, label] of Object.entries(COMPLIANCE_LABELS)) {
    ...
    item.className = "compliance-item" + (present ? " present" : " missing");
    item.innerHTML = `
      <div class="compliance-head">
        <span class="compliance-mark" aria-hidden="true">${present ? "✓" : "✗"}</span>
